...
def Task1():
    #Control one columns
    for i in range(50):
        LightUp(row_pins[0],col_pins[2])
        LightUp(row_pins[2],col_pins[2])
        LightUp(row_pins[3],col_pins[2])
        LightUp(row_pins[6],col_pins[2])

    #Arrow To The UP
    for i in range(50):
        LightUp(row_pins,col_pins[2])
        LightUp(row_pins[1],list(col_pins[i] for i in [1,3]))
        LightUp(row_pins[2],list(col_pins[i] for i in [0,4]))
    #Arrow To The Right
    for i in range(50):
        LightUp(row_pins[3],col_pins)
        LightUp(list(row_pins[i] for i in [2,4]),col_pins[3])
        LightUp(list(row_pins[i] for i in [1,5]),col_pins[2])        
